...
public class SparkMongoOutput extends SparkSinkSaveOutput implements SparkWriting, SparkMongo {
    ...
    @Override
    public void enqueue(Sdream<Rmap> sdream) {
        Maps.ofQ(sdream, Rmap::table).forEach(this::write);
    }
...
//          调用result的Count,求和
            int cc = result.getInsertedCount() + (result.isModifiedCountAvailable() ? result.getModifiedCount() : 0) + result.getUpserts().size();
